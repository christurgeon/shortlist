...
| **[R2] Cached research briefs** | `_financial_series` (`bridge.py:61-84`) gains a `shrs` cell, rendered by `assess.py:307-308`. Briefs are now keyed on a context digest that includes the rendered financial-series table (`research/cachekey.py`), so this table change generally busts the digest and regenerates on its own; `--refresh` still forces it immediately. |
