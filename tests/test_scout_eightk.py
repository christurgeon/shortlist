...
def test_firehose_carries_emission_meta():
    e = Emission("RBI", "edgar:8k", 0.6, "ev", True, "0000000007",
                 meta={"adsh": "a-1", "items": ["1.01", "3.03"]})
    ev = cohort_events_from_emissions([e], date(2026, 7, 3))[0]
    assert ev.meta == {"adsh": "a-1", "items": ["1.01", "3.03"]}
    assert ev.to_dict()["meta"]["adsh"] == "a-1"


def test_firehose_metaless_emission_unchanged():
    e = Emission("ABC", "edgar:activist_13d", 0.9, "ev", True)
    ev = cohort_events_from_emissions([e], date(2026, 7, 3))[0]
    assert ev.meta == {}
