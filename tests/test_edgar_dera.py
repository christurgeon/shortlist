...
from shortlist.edgar import dera
from shortlist.edgar.dera import dera_zip_url, load_index, parse_dera_tsvs, quarters_back
from shortlist.edgar.insider import parse_form4_xml
...
def test_load_index_round_trips_through_its_json_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("shortlist.edgar.dera.ensure_quarters",
                        lambda quarters, cache_dir, identity="x": [zip_path])
    monkeypatch.setattr("shortlist.edgar.dera._index_from_zip", spy_index_from_zip)
...
def test_load_index_never_caches_a_partial_result_and_retries_next_call(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("shortlist.edgar.dera.ensure_quarters", fake_ensure_quarters)
    cache_dir = str(tmp_path / "cache")
