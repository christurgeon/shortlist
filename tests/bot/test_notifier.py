import httpx
from shortlist.bot.notify import TelegramNotifier, deliver
...
def test_retries_on_429_then_succeeds(monkeypatch):
    slept = []
    monkeypatch.setattr("shortlist.bot.notify.time.sleep", lambda s: slept.append(s))
    seq = [429, 200]
...
def test_429_exhausts_retries_and_returns_false(monkeypatch):
    monkeypatch.setattr("shortlist.bot.notify.time.sleep", lambda s: None)
...
def test_get_updates_returns_results_on_200():
    from shortlist.bot.notify import TelegramNotifier
...
def test_send_message_chunks_by_utf16_units_for_astral_emoji():
    ...
    from shortlist.bot.notify import _MSG_CAP
    bodies = []
    n = TelegramNotifier("T", "42", client=_body_client(bodies))
...
def test_send_message_reuses_one_client_across_chunks(monkeypatch):
    ...
    import shortlist.bot.notify as notify
    created = []
