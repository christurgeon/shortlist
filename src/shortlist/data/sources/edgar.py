...
class EdgarSource(Source):
    ...
    def _build_financials_snapshot(
        ...
        try:
            ...
            # scale the filer's income statement uses -- MCD comes back as
            # [716.4, 721.9, 732.3], i.e. MILLIONS, not absolute shares (TODO.md §4).
            # It reaches exactly one consumer: extract_financials' computed-EPS
            # fallback (`ni / shares_diluted`), which fires only when no as-reported
            # EPS row was matched -- so a millions-scaled value there yields an EPS
            # 1e6x too small. `ef.diluted_shares` does NOT come from here (it is
            # extracted per-row, and the companyconcept fallback below is absolute),
            # so share_count_cagr and the dilution flag are unaffected. No sanity
            # bound is applied: every candidate threshold is fitted to this one
            # observation, and this repo does not ship n=1 heuristics.
